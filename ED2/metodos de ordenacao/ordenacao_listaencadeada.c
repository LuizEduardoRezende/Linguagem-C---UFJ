...
{
    tipoLista listai = lista;
    tipoLista listaj = lista ->proximo;
    int aux = 0;

    for(listai = lista; listai != NULL; listai = listai->proximo)
    {
        for(listaj = listai->proximo; listaj != NULL; listaj = listaj->proximo){
            if (listaj->item < listai->item)
            {
                aux = listaj->item;
                listaj->item = listai->item;
                listai->item = aux;
            }
        }
    }
...
void selectionsort(tipoLista lista)
{
    ...
    for(listai = lista; listai != NULL; listai = listai->proximo)
    {
        min = listai;
        for(listaj = listai->proximo; listaj != NULL; listaj = listaj->proximo){
            if (listaj->item < min->item)
                min = listaj;  
        }

        aux = listai->item;
        listai->item = min->item;
        min->item = aux;
    }
