...
void imprimir(tipoNoh *raiz, int espaco)
{
	...
	else
	{
		...
		printf("\n");
		int i = tamanhoDoEspaco;
		for (i = tamanhoDoEspaco; i < espaco; i++)
		{
			printf(" ");
		}
...
int inserir(tipoNoh **raiz, char pal[])
{
	...
	if (*raiz == NULL)
	{
		...
		strcpy(nohAuxiliar->item.palavra, pal);
	...
	if( strcmp(pal, nohAtual->item.palavra) < 0)
	{
		if ((controle = inserir(&(nohAtual->esquerda), pal)) == 1)
		{
			if (fatorBalanceamento(nohAtual) >= 2)
			{
				if( strcmp(pal,(*raiz)->esquerda->item.palavra) < 0)
					rotacaoDireita(raiz);
				else
	...
	else
	{
		if( strcmp(pal,nohAtual->item.palavra) > 0)
		{
			if ((controle = inserir(&(nohAtual->direita), pal)) == 1)
			{
				if (fatorBalanceamento(nohAtual) >= 2)
				{
					if( strcmp( pal, (*raiz)->direita->item.palavra) > 0)
						rotacaoEsquerda(raiz);
					else
...
int remover(tipoNoh **raiz, char pal[])
{
	int controle;
	...
	if( strcmp(pal, (*raiz)->item.palavra) < 0)
	{
		if ((controle = remover(&((*raiz)->esquerda), pal)) == 1)
		{
	...
	if(strcmp( pal, (*raiz)->item.palavra) > 0)
	{
		if ((controle = remover(&((*raiz)->direita), pal)) == 1)
		{
	...
	if(strcmp((*raiz)->item.palavra, pal) == 0)
	{
		...
		else
		{
			...
			strcpy((*raiz)->item.palavra, nohAuxiliar->item.palavra);
			remover(&(*raiz)->direita, (*raiz)->item.palavra);
			if (fatorBalanceamento(*raiz) >= 2)
			{
				if (alturaNoh((*raiz)->esquerda->direita) <= alturaNoh((*raiz)->esquerda->esquerda))
					rotacaoDireita(raiz);
